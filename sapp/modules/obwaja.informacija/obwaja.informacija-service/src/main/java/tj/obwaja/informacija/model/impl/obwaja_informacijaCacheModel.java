...
/**
 * The cache model class for representing obwaja_informacija in entity cache.
 *
 ...
 */
@ProviderType
public class obwaja_informacijaCacheModel implements CacheModel<obwaja_informacija>,
	Externalizable {
	@Override
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof obwaja_informacijaCacheModel)) {
			return false;
		}

		obwaja_informacijaCacheModel obwaja_informacijaCacheModel = (obwaja_informacijaCacheModel)obj;

		if (obwaja_informacija_id == obwaja_informacijaCacheModel.obwaja_informacija_id) {
			return true;
		}
	...
	@Override
	public obwaja_informacija toEntityModel() {
		obwaja_informacijaImpl obwaja_informacijaImpl = new obwaja_informacijaImpl();

		if (data_izmenenija == Long.MIN_VALUE) {
			obwaja_informacijaImpl.setData_izmenenija(null);
		}
		else {
			obwaja_informacijaImpl.setData_izmenenija(new Date(data_izmenenija));
		}

		if (data_sozdanija == Long.MIN_VALUE) {
			obwaja_informacijaImpl.setData_sozdanija(null);
		}
		else {
			obwaja_informacijaImpl.setData_sozdanija(new Date(data_sozdanija));
		}

		obwaja_informacijaImpl.setIzmenil(izmenil);
		obwaja_informacijaImpl.setIzvewenie_id(izvewenie_id);

		if (je_pochta == null) {
			obwaja_informacijaImpl.setJe_pochta(StringPool.BLANK);
		}
		else {
			obwaja_informacijaImpl.setJe_pochta(je_pochta);
		}

		if (kontaktnoe_lico == null) {
			obwaja_informacijaImpl.setKontaktnoe_lico(StringPool.BLANK);
		}
		else {
			obwaja_informacijaImpl.setKontaktnoe_lico(kontaktnoe_lico);
		}

		if (kontaktnyj_telefon == null) {
			obwaja_informacijaImpl.setKontaktnyj_telefon(StringPool.BLANK);
		}
		else {
			obwaja_informacijaImpl.setKontaktnyj_telefon(kontaktnyj_telefon);
		}

		obwaja_informacijaImpl.setObwaja_informacija_id(obwaja_informacija_id);
		obwaja_informacijaImpl.setSozdal(sozdal);

		obwaja_informacijaImpl.resetOriginalValues();
